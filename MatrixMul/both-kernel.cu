...
int main(){
    ...
    int K = genrangerandint(MAX_LEN, MIN_LEN);
    ...
    genrandmat(genrangerandint, B, N, K);
    memset(C, 0, SizeC);
    ...
    showmat(B, N, K, "B");
    showmat(C, M, K, "C");
    showmat(HostRef, M, K, "HostRef C");

    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, \
        M, K, N, \
        (MYTYPE)1.0, A, N, \
        B,K, (MYTYPE)0.0, HostRef, K);
    showmat(HostRef, M, K, "HostRef C");
    ...
    showmat(C, M, K, "device C using matmul kernel");
    ...
    cudaDeviceSynchronize();
    cudaMemcpy(C, dC, SizeC, cudaMemcpyDeviceToHost);
    showmat(C, M, K, "device C using tiled_matmul kernel");
